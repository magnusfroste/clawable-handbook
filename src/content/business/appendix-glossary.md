...
| Term | Definition |
|------|------------|
| **OpenClaw** | The open-source agent runtime Peter Steinberger released in November 2025; by early 2026 one of the fastest-growing open-source projects ever, with roughly half a million systems running it worldwide. The reference architecture where the agent's identity, schedule, and memory live in plain text files a human can read and correct. |
| **AgentHotel** | The self-hosted control panel that runs several AI agents side by side on one server — each isolated, resource-capped and reachable on its own domain. Formerly ClawClass, which hosted the operator deployments documented in this handbook. |
| **Clawable** | The reference external operator in this handbook. An OpenClaw instance deployed on ClawClass (today AgentHotel) that reads FlowWink via MCP. |
