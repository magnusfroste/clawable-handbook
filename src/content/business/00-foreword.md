...
By 2025 the shape had appeared in full — not in how we communicate, not in how we send value, but in how businesses operate: models that could act, not just answer. So I did what I have done every time before, except I went further: I built the business platform (FlowWink), built the infrastructure to run a fleet of agents (AgentHotel), connected autonomous operators to a live business, and logged everything — including what broke. This handbook is the write-up of that experiment. Since the start of 2025, it has had my full-time focus: agentic AI and AI inference, and nothing else.

Thirty years after that thesis, the professor's objection still stands: nobody can fully explain how these systems reason. The capability this book is about — reasoning that emerges from training at enormous scale — still cannot be read off its weights. What has changed is not the explanation. It is what we do instead of one: log what the system does, verify the outcome against evidence, and govern its actions rather than pretend to understand its mechanism. That is the answer I did not have in 1996. It is most of what this book is.
