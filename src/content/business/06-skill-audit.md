...
**Absent.** The platform's own onboarding brief named a lead-listing tool as a key capability. No tool by that name existed — the capability lived under a different name. If the platform's documentation cannot find the tool, neither can the agent.
...
The industry is converging on an answer, and it is strikingly unglamorous: plain, well-organized text files that a human and an agent can both read. Google published an open standard for exactly this in June 2026 — the **Open Knowledge Format**. `validated` It is the same pattern this handbook's operators already run on: the mandate in a text file, the company profile the agent fetches on demand, memory a human can correct in an ordinary editor. Knowledge as files is the pattern the agent era keeps rediscovering, because it is the only format that serves both readers at once.
