...
1. **Circuit breakers** — stop failing skills from cascading into system-wide instability.
2. **Retry discipline** — exponential backoff with jitter for transient failures, fail fast for permanent errors.
3. **Escalation visibility** — quarantine unstable skills and notify humans with actionable context.
...
*Next: the failure mode nobody talks about — recovering from hallucinated tool calls. [Tool Hallucination Recovery →](/builder/32-tool-hallucination-recovery)*
