...
description: "How an agent with 500+ skills picks the right one every time — synonym expansion, trigger matching, historical boosting, and why it works in Swedish."
order: 17
icon: "target"
---
...
With 10 skills, you can. With 500+, you can't — for three reasons:

1. **Token cost** — 500+ skills × ~97 tokens metadata each = ~50K tokens. With intent filtering, it's 25 skills × ~97 = ~2.4K tokens. Over 95 percent of the metadata cost removed, on every request.
...
3. **Hallucination reduction** — More tools in the prompt means more names for the model to confuse. Reducing from 500+ to 25 cuts the surface area for phantom tool calls dramatically.
...
*Next: the memory system that gives the agent continuity across sessions. [Memory Architecture →](/builder/18-memory-architecture)*
