...
| **Harness** | The industry's 2026 term for the scaffolding that turns a model into an agent — the loop, tool dispatch, permissions, context management, memory, tracing. What this handbook calls the **control plane** — mapped into eleven components in its own chapter (5b). OpenClaw is a general-purpose harness; FlowPilot's heartbeat loop is an embedded one. |
...
| Term | Definition |
|------|-----------|
| **AgentHotel** | Self-hosted control panel for running a fleet of AI agents on one server — Hermes, OpenClaw, Odysseus, or any Docker image. Each agent gets an isolated, resource-capped container with its own domain and automatic TLS; an MCP server lets external AI tools create agents and delegate work (`ask_agent`). Formerly ClawClass. See chapter 33. |
| **Paperclip** | An orchestration layer that represents the organization — the CEO level — delegating tasks to individual agents the way a manager delegates to employees. |
| **Caddy** | The web server used by AgentHotel. Handles automatic TLS certificate issuance on first request. No nginx, no certbot, no manual cert management. |
...
*This glossary is updated as the handbook evolves. If a term is missing, [reach out on GitHub](https://github.com/magnusfroste).*
